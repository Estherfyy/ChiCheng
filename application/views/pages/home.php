<div class="banner"></div>
<div class="section1">
  <div class="concenter">
    <ul>
      <li>
        <div class="hove_box"> <span class="red_bizhi">更专业</span>
          <p style="font-size:18px; color:#fff; line-height:100%; font-weight:bold; margin-bottom:14px">注册前咨询</p>
          <div style="padding:0 9px 0 0;">针对客户的实际情况，综合考虑税收、股权结构、维护成本、可扩展性等，我们集十年的注册公司经验，提供符合客户实际需要的咨询意见。这种意见是有价值的，
            一般代理公司无法提供的&nbsp;。我们酝酿将来对这部分服务单独收费。</div>
        </div>
        <span class="red_bizhi">更专业</span>
        <div class="li_conbox"> <img src="images/section1_img1.png" width="147" height="41" style="display:block; margin:0 auto 4px;" /> <span>10年经验，我们更懂你</span> </div>
      </li>
      <li>
        <div class="hove_box"> <span class="red_bizhi">更省钱</span>
          <p style="font-size:18px; color:#fff; line-height:100%; font-weight:bold; margin-bottom:14px">代理记账</p>
          <div>现在您不必烦恼了！拨个电话给我们，专业会计师为您服务！</div>
        </div>
        <span class="red_bizhi">更省钱</span>
        <div class="li_conbox"> <img src="images/section1_img2.png" width="118" height="42" style="display:block; margin:0 auto 4px;" /> <span>为您考虑，为您更省</span> </div>
      </li>
      <li>
        <div class="hove_box"> <span class="red_bizhi">更快捷</span>
          <p style="font-size:18px; color:#fff; line-height:100%; font-weight:bold; margin-bottom:14px">专业管理，流程极简</p>
          <div>一份合同，一次签字，一家公司，一次搞定。8大项22小项操作流程审核，每周经理项目review，每个环节和每个员工个人绩效考核的挂钩，奠定了超过同行手工作坊的服务质量。繁在心，简在形。 </div>
        </div>
        <span class="red_bizhi">更快捷</span>
        <div class="li_conbox"> <img src="images/section1_img3.png" width="118" height="78" style="display:block; margin:0 auto 4px;" /> <span>资料一次提交<br/>
          其余的交给我们</span> </div>
      </li>
      <li>
        <div class="hove_box"> <span class="red_bizhi">更省心</span>
          <p style="font-size:18px; color:#fff; line-height:100%; font-weight:bold; margin-bottom:14px">服务完整维护成本低</p>
          <div> 我们是全国最大的中小企业服务中心。不仅在公司注册环节，更在公司注册之后，提供年检、财务做账报税、人事外包、法律服务、商标注册等外包服务，全面的企业一站式保姆服务。关注您的核心业务，将行政事务性工作都交给我们！</div>
        </div>
        <span class="red_bizhi">更省心</span>
        <div class="li_conbox"><img src="images/section1_img4.png" width="149" height="78" style="display:block; margin:0 auto 4px;" /> <span>让您无后顾之忧</span> </div>
      </li>
    </ul>
  </div>
</div>

<div class="section2">
  <div class="concenter"> <img src="images/section2_tt.png" />
    <h3 class="h3_line">我们不止为您想的更多，为您做的更多</h3>
    <p>由于取消了年检，工商改为要求企业每年网络公示，否则将会被列入黑名单。包括公司运营维护、财务人事外包、各种变更注销、行业准入咨询等等。我们提供运营公司所需的相关支撑业务。关注您的核心业务，把行政事务性工作交给企盈。</p>
  </div>
</div>

<div class="section3">
  <div class="concenter">
    <h3 class="h3_tt"><img src="images/section3_tt.png" /></h3>
  </div>
</div>

<div class="section4">
  <div class="concenter">
    <h3 class="h3_tt"><img src="images/section4_tt.png" style="display:block; margin:0 auto 13px;" /> <span class="span_en">OTHER SERVICES</span></h3>
    <div class="ul_box">
      <ul>
        <li>
          <div class="hover_div">
            <div>
              <p style="font-size:18px;font-weight:bold; color:#fff; margin-bottom:10px;">财务代理</p>
              专业的财务代理公司，有超过60名专业的财务人员。被财政局评为上海财务会计信用等级B级单位。2014年9月，被上海市经济和信息化委员会认定为"上海市（市级）中小企业服务机构"。 </div>
          </div>
          <div class="li_con">
            <div> <img src="images/section4_img1.png" />
              <p class="ch_p">财务代理</p>
              <p class="en_p">Deputy Finance</p>
            </div>
           </div>
        </li>
        <li>
          <div class="hover_div">
            <div>
              <p style="font-size:18px;font-weight:bold; color:#fff; margin-bottom:10px;">人事代理</p>
              为上海企业办理人员招退工手续、社保公积金缴纳、居住证积分、居转户和留学人员落户、外国人三证等业务。专业的咨询师和经验丰富的操作人员进行一对一服务，将企业从烦杂的人事事务中解脱出来，最大化减少成本，全身心的投入到经营中去。 </div>
          </div>
          <div class="li_con">
            <div> <img src="images/section4_img2.png" />
              <p class="ch_p">人事代理</p>
              <p class="en_p">Personnel Agency</p>
            </div>
          </div>
        </li>
        <li>
          <div class="hover_div">
            <div>
              <p style="font-size:18px;font-weight:bold; color:#fff; margin-bottom:10px;">商标注册</p>
              当天递交申请，超过10000家成功案例一对一咨询师服务，尽职查询，高注册通过率，实时跟踪与监测，让您无后顾之忧。 </div>
          </div>
          <div class="li_con">
            <div> <img src="images/section4_img3.png" />
              <p class="ch_p">商标注册</p>
              <p class="en_p">Trademark registration</p>
            </div>
          </div>
        </li>
      </ul>
    </div>
  </div>
</div>

<div class="tanchu_opac">
</div>


<!--[if IE 6]>
    <style type="text/css">
        body{behavior:url("/css/csshover.htc");}
    </style>
    <script type="text/javascript" src="/js/IEpng.js"></script>
    <div id="ie6-warning" style="background:#ED1B23;position:absolute;top:0;left:0;font-size:12px;line-height:24px;color:#fff;padding:0 10px;z-index:9999;vertical-align:baseline;">
    <span width="14" height="14" style="float:right;cursor:pointer;margin-top:4px; font-size:14px;text-align:center;" onclick="document.getElementById('ie6-warning').style.display ='none';" alt="关闭提示" />&times</span>
    <span>您正在使用 Internet Explorer 6 低版本的IE浏览器。为更好的浏览本页，建议您将浏览器升级到</span>
    <a style="color:#fff;" href="http://windows.microsoft.com/zh-CN/windows/upgrade-your-browser" target="_blank">IE8</a>
     或以下浏览器：<a style="color:#fff;" href="http://www.google.cn/intl/zh-CN/chrome/" target="_blank">Chrome</a> / <a style="color:#fff;" href="http://www.firefox.com.cn/download/" target="_blank">Firefox</a> / <a style="color:#fff;" href="http://support.apple.com/kb/DL1531?viewlocale=zh_CN" target="_blank">Safari</a> / <a style="color:#fff;" href="http://www.opera.com/" target="_blank">Opera</a>
    </div>
<![endif]-->

<script type="text/javascript" src="js/jquery.js"></script>
<script type="text/javascript" src="js/html5.js"></script>
<script type="text/javascript" src="js/main.js"></script>
<script type="text/javascript" src="js/rightFloat.js"></script>
<script type="text/javascript" src="js/jquery.easing.1.3.js"></script>
<script type="text/javascript" src="js/post.js"></script>


<div class="xuanfu">
  <ul>
  	<li class="dh_btn" title="电话">
      <img src="images/tel_btn.png" style="" /> <em class="tm">&nbsp;</em>
    </li>
    <li class="qqBtn" title="咨询在线专家">
      <a class="talk99_region" target="_blank" > <img alt="" src="images/qq_btn.png" /></a>
    </li>
  </ul>

<div class="liuyan"> <span class="gb"><img src="images/gb.png" /></span>
  <div class="liuyan-nr">
    <h3>在线专家咨询</h3>
    <div class="dlBox">
      <p style="line-height:15px; margin:0 0 10px; font-weight:bold;">工作日：8:30-17:30</p>

      <dl>
        <dt> <a class="talk99_region" target="_blank" href="tencent://Message/?Uin=1002859078" > <img alt="" src="images/t2.jpg" /></a></dt>
        <dd> <a class="talk99_region" target="_blank" href="tencent://Message/?Uin=1002859078">高经理</a></dd>
      </dl>
      <dl>
        <dt> <a class="talk99_region" target="_blank" href="tencent://Message/?Uin=106245924" > <img alt="" src="images/t2.jpg" /></a></dt>
        <dd> <a class="talk99_region" target="_blank" href="tencent://Message/?Uin=106245924">程经理</a></dd>
      </dl>
      <dl>
        <dt> <a class="talk99_region" target="_blank" href="tencent://Message/?Uin=575250937" > <img alt="" src="images/t2.jpg" /></a></dt>
        <dd> <a class="talk99_region" target="_blank" href="tencent://Message/?Uin=575250937">郭经理</a></dd>
      </dl>
    </div>
  </div>
</div>



  <div class="tel_neirong">
  	<span class="gb"><img src="images/gb.png" /></span>
    <div class="liuyan-nr">
      <div class="tel-bt"><img src="images/tel_btn.png" />联系电话</div>
      <div class="dlBox">
	  	  <p class="tel_p" style="margin:51px auto 5px;">4000-562-618</p>
          <div class="tip" style="margin:0 0 10px 0;">
          	  <div>（限工作日：8:30-17:30）</div>
          </div>
          <p class="tel_p" style=" font-size:46px;margin:0 auto 4px;">1881-7502-679</p>
          <div class="tip"><div>（限工作日：8:30-17:30）</div></div>
      </div>
    </div>
  </div>

</div>
<script src="js/jquery.superslide.2.1.1.js"></script>
<script>
	$(function(){
		$('.section1 li').each(function() {
			var div_h=(248-$(this).children('.li_conbox').height())/2;
			$(this).children('.li_conbox').css({'margin-top':div_h});
		});
		$(".partenr_effect").slide({titCell:".hd ul",mainCell:".bd>ul",autoPage:true,effect:"left",interTime:6000,mouseOverStop:false,autoPlay:true,vis:1,scroll:1,trigger:"mouseenter",autoPage:"<li><a></a></li>"});
		//section1手表接经过效果上的四点
		$('.section1 ul li').hover(function(){
			$(this).find('.hove_box').stop(true,true).fadeIn(270);
		},function(){
			$(this).find('.hove_box').stop(true,true).fadeOut(100);
		});
	});
</script>
